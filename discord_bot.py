...
async def refresh_live_trades_channel():
    ...
    try:
        ...
        logger.error(f"Could not connect to Flask app: {e}")
        return
    ...
    logger.info("Change in active trades detected. Refreshing channel.")
    if not trades:
    ...
    logger.info("Active trades channel refreshed.")
...
async def status(interaction: discord.Interaction):
    ...
    try:
        ...
        await interaction.followup.send(embed=discord.Embed.from_dict(STATUS_EMBED["unreachable"]), ephemeral=True)
...
async def bot_command(interaction: discord.Interaction, action: app_commands.Choice[str]):
    await interaction.response.defer(ephemeral=True)
    endpoint = f"/start_trading" if action.value == "start" else f"/stop_trading"
    try:
        ...
        if not balances:
            ...
            for account_name, balance_data in balances.items():
                if "error" in balance_data:
                    ...
                    value = "\n".join([f"**{code.upper()}:** `{amount}`" for code, amount in filtered_balances.items()]) if filtered_balances else "No active balances."
...
@app_commands.describe(month="Month for the summary (e.g., 'August 2023'). Defaults to current.")
async def bitso(interaction: discord.Interaction, month: str = None):
    await interaction.response.defer(ephemeral=True)
    try:
        response = requests.get("http://127.0.0.1:5001/bitso_summary", params={"month": month} if month else None, timeout=30)
        if response.status_code == 200:
            data = response.json()
            if data.get("success"):
...
@app_commands.describe(month="Month for the report (e.g., 'August 2023')")
async def bitso_chart(interaction: discord.Interaction, month: str = None):
    ...
    try:
        ...
        if not all_fundings:
            await interaction.followup.send(f"No Bitso funding data for {target_date:%B %Y}.", ephemeral=True)
            return
...
async def charts(interaction: discord.Interaction):
    ...
    try:
        ...
        if response.status_code == 200 and response.json().get("success"):
            chart_paths = response.json().get("charts", {})
            ...
            await interaction.followup.send(f"Error: {response.json().get('error', 'Unknown error.')}", ephemeral=True)
    except requests.exceptions.RequestException:
        await interaction.followup.send(SERVER_UNREACHABLE, ephemeral=True)
...
@app_commands.describe(offer_hash="The hash of the offer to toggle", account_name="The account that owns the offer", status="The desired status")
...
@app_commands.describe(trade_hash="The trade hash", account_name="The account handling the trade", message="The message to send")
