...
@tree.command(name="list_offers", description="Lists all of your active offers.")
async def list_offers_command(interaction: discord.Interaction):
    """Handles the /list_offers slash command."""
    await interaction.response.defer(ephemeral=True)
    try:
        ...
        embed = discord.Embed(
            title=f"Your Active Offers ({len(offers)})",
            color=COLORS["info"]
        )

        for offer in offers[:20]: # Show up to 20 offers to avoid hitting Discord limits
            status = "✅ On" if offer.get('enabled') else "❌ Off"
            offer_name = f"{offer.get('payment_method_name', 'N/A')} ({offer.get('account_name', 'N/A')})"
            offer_details = (
                f"**Margin**: {offer.get('margin', 'N/A')}%\n"
            ...
            embed.add_field(name=offer_name, value=offer_details, inline=False)
        
        await interaction.followup.send(embed=embed, ephemeral=True)

    except requests.exceptions.RequestException as e:
        await interaction.followup.send(SERVER_UNREACHABLE, ephemeral=True)
        logger.error(f"Could not connect to Flask app for /list_offers: {e}")

@tree.command(name="toggle_offer", description="Turn a specific offer on or off.")
@app_commands.describe(
    offer_hash="The hash of the offer to toggle",
    account_name="The account that owns the offer (e.g., David_Noones)",
...
async def toggle_offer_command(interaction: discord.Interaction, offer_hash: str, account_name: str, status: app_commands.Choice[str]):
    """Handles the /toggle_offer slash command."""
    await interaction.response.defer(ephemeral=True)
    ...
    try:
        ...
        if response.status_code == 200 and data.get("success"):
            embed = discord.Embed(
                title=f"✅ Offer Status Updated",
                description=f"Successfully turned **{status.name}** the offer `{offer_hash}`.",
                color=COLORS["success"] if is_enabled else COLORS["error"]
            )
        else:
            embed = discord.Embed(
                title=f"❌ Error Updating Offer",
                description=f"Failed to update offer `{offer_hash}`.\n**Reason**: {data.get('error', 'Unknown server error.')}",
                color=COLORS["error"]
            )
