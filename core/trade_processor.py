...
def process_trades(account):
    ...
    if not gmail_service:
        logger.warning("Failed to initialize Gmail service. Email checking will be disabled for this session.")

    access_token = fetch_token_with_retry(account)
    if not access_token:
        logger.error(f"Failed to fetch access token for {account['name']}. Skipping account.")
        return
        ...
        if trades:
            for trade in trades:
                ...
                if not trade_hash or not owner_username:
                    logger.error(f"Missing trade_hash or owner_username for trade: {trade}")
                    continue
                ...
                if trade_status not in processed_trade_data.get('status_history', []):
                    logger.info(f"Trade {trade_hash} has a new status: '{trade_status}'")
                ...
                email_check_is_relevant = payment_method_slug in ["oxxo", "bank-transfer", "spei-sistema-de-pagos-electronicos-interbancarios", "domestic-wire-transfer"]
                ...
                if is_paid and email_check_is_relevant and is_pending_email_check and 'paid_timestamp' in processed_trade_data:
                    elapsed_time = datetime.now(timezone.utc).timestamp() - processed_trade_data['paid_timestamp']

                    if elapsed_time < EMAIL_CHECK_DURATION:
                        logger.info(f"Trade {trade_hash} is Paid. Checking for confirmation email for {payment_method_slug} ({int(elapsed_time)}s elapsed)...")
                        email_check_successful = check_for_payment_email(gmail_service, trade)
                        
                        if email_check_successful:
                            logger.info(f"PAYMENT VERIFIED via email for trade {trade_hash}.")
                            send_email_validation_alert(trade_hash, success=True)
                            processed_trade_data['email_verified'] = True
                            save_processed_trade(trade, platform, processed_trade_data)
                    else:
                        logger.warning(f"Email check for trade {trade_hash} timed out after {EMAIL_CHECK_DURATION} seconds.")
                ...
                if trade_status.startswith('Active') and not processed_trade_data.get('reminder_sent'):
                    ...
                    if last_buyer_ts:
                        ...
                        try:
                            ...
                            logger.error(f"Could not parse start_date '{started_at_str}' for trade {trade_hash}.")
                    
                    if reference_time:
                        if (datetime.now(timezone.utc) - reference_time).total_seconds() > PAYMENT_REMINDER_DELAY:
                            logger.info(f"Sending payment reminder for trade {trade_hash} due to inactivity.")
                            send_payment_reminder_message(trade_hash, account, headers)
                            processed_trade_data['reminder_sent'] = True
                            save_processed_trade(trade, platform, processed_trade_data)
                    else:
                        logger.warning(f"Cannot check for reminder for trade {trade_hash}: no valid reference time.")

                if attachment_found and not processed_trade_data.get('attachment_message_sent'):
                    logger.info(f"New attachment found for trade {trade_hash}. Sending one-time message.")
                    ...
                    if new_attachment_paths:
                        for attachment_path in new_attachment_paths:
                            ...
                            logger.info(f"Performing OCR on {attachment_path}...")
            ...
            logger.debug(f"No new trades found for {account['name']}.")
